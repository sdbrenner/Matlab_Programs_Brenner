function [k,err] = vect_wavenum(omega,depth,tolerance,num_iterations)
% Linear dispersion relation wavenumber solver appropriate for arrays of 
% 'omega' and 'depth'.
% 
% k = vect_wavenum(omega,depth) solves for the wave-numbers 'k' for a given
% depth that satisfy the dispersion relation:
%           omega^2 = k*tanh(k*depth) 
% using the Newton-Raphson method.
%
% k = vect_wavenum(omega,depth,tolerance) assigns a tolerance value for
% this calculation. 
% If 'tolerance' is omitted, or entered as [], then a default value of 1e-6 is used.  
% The solver continues iterating until all values of 'k' meet this
% tolerance level or until it reaches the maximum number of iterations.
%
% k = vect_wavenum(omega,depth,tolerance,num_iterations) computes 'k'
% within a given tolerance using a maximum number of interations:
% 'num_iterations'. 
% If 'num_iterations' is omitted, or entered as [], then a default value of 10 is used.  
%
% [k,err] = vect_wavenum(omega,depth,...) returns the approximation error 
% associated with each value 'k':
% err = abs( k*tanh(k*d) - omega^2 )

% Samuel Brenner, April 2018
...
% If omega or depth are scalars, they should be converted to vectors of
% equal size:
if length(omega(:)) == 1
...
% Initial guess from CEM II-1-11:
% kn = (w.^2/g) .* ( tanh( (w.^2/g).*d ) ).^(-1/2)

%% Initial guess from Fenton and McKee, 1990:
k0 = omega.^2/g; % deep water wavenumber
kn = k0 .* ( tanh((k0.*depth).^(3/4)) ).^(-2/3);
err = abs( omega.^2 - g*kn.*tanh(kn.*depth) ); % error for initial guess
